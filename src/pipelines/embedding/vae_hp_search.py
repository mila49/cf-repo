"""Joint W&B sweep for VAE, GAT, Leiden and signature coherence.

Each run:
1. trains one variational autoencoder with early stopping;
...
7. optimises the same composite_score used by the AE sweep.

The best configuration and summary are saved under outputs/vae_sweep/.
"""
...
HP_CONFIG_PATH = PROJECT_ROOT / "config" / "embeddings" / "vae_hp_search.yml"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "vae_sweep_noGAT"
...
def train_vae(pipeline, train_loader, val_loader, val_size):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        wandb.log(
            {
                "epoch": epoch + 1,
                "train_vae_loss_epoch": avg_train_loss,
                "val_vae_loss_epoch": avg_val_loss,
            }
        )

        print(
            f"Epoch {epoch + 1:03d}/{MAX_EPOCHS} | "
            f"Train VAE loss: {avg_train_loss:.6f} | "
            f"Val VAE loss: {avg_val_loss:.6f} | "
            f"Best Val VAE loss: {best_val_loss:.6f}"
        )
    ...
    if best_model_state is None:
        raise RuntimeError("No valid VAE state was saved during training.")

    pipeline.model.load_state_dict(best_model_state)
    return best_train_loss, best_val_loss, best_epoch
...
def sweep_worker():
    ...
    try:
        ...
        pipeline = VAEPipeline(config_path="embeddings/vae_embedding.yml")
        ...
        best_train_loss, best_val_loss, best_epoch = train_vae(
            pipeline,
            train_loader,
            val_loader,
            val_size,
        ...
        final_metrics = {
            "train_vae_loss": best_train_loss,
            "val_vae_loss": best_val_loss,
        ...
        print(f"  Validation VAE loss:    {best_val_loss:.6f}")
...
def save_best_run(best_run):
    ...
    config_path = OUTPUT_DIR / "best_vae_leiden_config.yml"
    summary_path = OUTPUT_DIR / "best_vae_leiden_summary.yml"
