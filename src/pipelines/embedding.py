...
from src.utils.clustering.leiden import main as run_leiden
from src.utils.clustering.clusters_comparison import main as compare_clusters


class VaeClusteringPipeline:
    def __init__(self, config_path: str, preprocessing_config_path: str = "config/preprocessing.yml"):
        self.config = self.load_config(config_path)
        self.preprocessing_config = self.load_config(preprocessing_config_path)
    ...
    def setup_data(self):
        pc = self.preprocessing_config
        self.adata = load_data(pc["data_path"])

        self.adata = preprocess_data(
            self.adata,
            n_top_genes=pc["n_top_genes"],
            min_genes=pc["min_genes"],
            min_cells=pc["min_cells"],
            max_genes=pc["max_genes"],
            max_pct_mt=pc["max_pct_mt"],
            target_sum=pc["target_sum"],
            hvg_flavor=pc["hvg_flavor"],
